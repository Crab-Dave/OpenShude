...
(async () => {
  ...
  assert.equal(await desktop.locator('.sidebar-brand .brand-mark').textContent(), '宿');
  ...
  await desktop.waitForSelector('.activity-month-grid');
  assert.equal(await desktop.locator('.activity-month-day.outside').first().evaluate((element) => getComputedStyle(element).backgroundColor), 'rgb(238, 234, 243)');
  ...
  const currentUser = desktop.locator('#activity-group-form .candidate', { hasText: '林夏（我）' });
  assert.equal(await currentUser.locator('input').isChecked(), true);
  assert.equal(await currentUser.locator('input').isDisabled(), true);
  const activityGroupSearch = desktop.locator('#activity-group-form [data-person-search="activity-group"]');
  await activityGroupSearch.fill('browser-page-11');
  assert.equal(await desktop.locator('#activity-group-form .candidate:visible').count(), 1);
  assert.match(await desktop.locator('#activity-group-form .candidate:visible').textContent(), /分页同学11.*2026级.*计算机科学与技术/s);
  await desktop.locator('#activity-group-form [data-select-visible="activity-group"]').click();
  assert.equal(await desktop.locator('#activity-group-form .candidate:visible input').isChecked(), true);
  await activityGroupSearch.fill('视觉传达设计');
  const visibleActivityMembers = desktop.locator('#activity-group-form .candidate:visible input:not(:disabled)');
  ...
  assert.equal(await desktop.locator('.activity-target-groups input').first().evaluate((input) => getComputedStyle(input).width), '14px');
  assert.equal(await desktop.locator('input[type="checkbox"]').first().evaluate((input) => getComputedStyle(input).height), '14px');
  ...
  const selectionGroupSearch = admin.locator('#selection-group-form [data-person-search="selection-group"]');
  await selectionGroupSearch.fill('browser-page-11');
  assert.equal(await admin.locator('#selection-group-candidates .candidate:visible').count(), 1);
  assert.match(await admin.locator('#selection-group-candidates .candidate:visible').textContent(), /分页同学11.*计算机科学与技术.*browser-page-11/s);
  await admin.locator('#selection-group-form [data-select-visible="selection-group"]').click();
  assert.equal(await admin.locator('#selection-group-candidates .candidate:visible input').isChecked(), true);
  await selectionGroupSearch.fill('计算机科学与技术');
  const visibleSelectionMembers = admin.locator('#selection-group-candidates .candidate:visible input');
  assert.ok((await visibleSelectionMembers.count()) > 1);
  await admin.locator('#selection-group-form [data-select-visible="selection-group"]').click();
  assert.equal(await visibleSelectionMembers.evaluateAll((inputs) => inputs.every((input) => input.checked)), true);
  assert.equal(await admin.locator('#selection-group-candidates .candidate', { hasText: '陈遇' }).locator('input').isChecked(), false);
  await admin.locator('#selection-group-form [name="isPublic"]').check();
  ...
  assert.match(await browserSelectionGroup.textContent(), /已公开共享/);
  ...
  await configuredGroup.filter({ hasText: '林夏' }).waitFor();
  assert.match(await configuredGroup.textContent(), /林夏/);
