...
(async () => {
  ...
  await activityGroupSearch.fill('browser-page-10');
  assert.equal(await desktop.locator('#activity-group-form .candidate:visible').count(), 1);
  assert.match(await desktop.locator('#activity-group-form .candidate:visible').textContent(), /分页同学10.*2026级.*计算机科学与技术/s);
  ...
  await selectionGroupSearch.fill('browser-page-10');
  assert.equal(await admin.locator('#selection-group-candidates .candidate:visible').count(), 1);
  assert.match(await admin.locator('#selection-group-candidates .candidate:visible').textContent(), /分页同学10.*计算机科学与技术.*browser-page-10/s);
