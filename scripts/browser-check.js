...
(async () => {
  ...
  await browserSelectionGroup.locator('[data-export-selection-group]').click();
  const groupCardDownload = await groupCardDownloadPromise;
  assert.match(groupCardDownload.suggestedFilename(), /^浏览器验收学生群组-cards-\d{4}-\d{2}-\d{2}\.xlsx$/);
  await admin.locator('.modal [data-close]').click();
