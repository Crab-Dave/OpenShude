...
const baseActivities = [
  ...
  ...[
    ...
    location, organizer, type: index === 14 ? 'official' : 'personal', importance: index === 14 ? 4 : (index % 5 === 0 ? 2 : 1),
...
function renderMonth() {
  ...
  const cells = Array.from({ length: 42 }, (_, index) => {
    ...
    return `<div class="${classes}" role="button" tabindex="0" data-open-day="${dateKey(date)}" aria-label="${date.getMonth() + 1} 月 ${date.getDate()} 日，${events.length} 个活动"><div class="month-day-head"><span class="month-day-number">${date.getDate()}</span>${events.length ? `<span class="day-total">${events.length} 个</span>` : ''}</div>${events.slice(0, 3).map((activity) => `<div class="month-event importance-${activity.importance}"><time>${activity.allDay ? '全天' : timeLabel(activity).slice(0, 5)}</time><span>${escapeHtml(activity.title)}</span></div>`).join('')}${events.length > 3 ? `<div class="month-more" data-count="${events.length}">另有 ${events.length - 3} 个活动</div>` : ''}</div>`;
  }).join('');
  return `<section class="panel calendar-panel"><div class="calendar-weekdays">${['周一','周二','周三','周四','周五','周六','周日'].map((name) => `<span>${name}</span>`).join('')}</div><div class="month-grid">${cells}</div></section>`;
}
...
function renderDay() {
  ...
  const groupMarkup = groups.map((group) => {
    ...
    return `<section class="time-group"><div class="time-label"><strong>${time}</strong><span>${group.events.length} 个${group.events.length > 1 && !group.allDay ? '重叠' : ''}活动</span></div><div class="time-events">${visible.map((activity) => renderActivityCard(activity)).join('')}</div>${group.events.length > 3 ? `<div class="time-group-actions"><button class="btn btn-soft btn-sm" data-expand-slot="${slotKey}">${expanded ? '收起' : `展开其余 ${group.events.length - 3} 个`}</button></div>` : ''}</section>`;
  }).join('');
...
function renderDetail() {
  ...
  const joined = isJoined(activity);
  const percent = Math.min(100, Math.round(count / activity.capacity * 100));
  const statusLabel = activity.status === 'DRAFT' ? '草稿' : isEnded(activity) ? '已结束' : '报名中';
  const people = [...(activity.people || [])];
  if (joined && !people.includes('林夏')) people.unshift('林夏');
  document.querySelector('#detail-page').innerHTML = `<div class="page-actions"><button class="btn" data-go-calendar>${icon('arrow-left')}返回活动广场</button><button class="btn push" data-template="${activity.id}">${icon('copy')}以此为模板</button><button class="btn" data-copy-link>${icon('share-2')}复制链接</button></div><section class="panel detail-hero"><div><div class="card-top">${activity.type === 'official' ? '<span class="badge badge-official">官方活动</span>' : '<span class="badge">个人活动</span>'}<span class="badge">${importanceLabel(activity.importance)}</span><span class="badge ${activity.status === 'DRAFT' ? 'badge-warning' : 'badge-success'}">${statusLabel}</span></div><span class="eyebrow">CAMPUS EVENT</span><h2>${escapeHtml(activity.title)}</h2><p class="summary">${escapeHtml(activity.description.split('\n')[0])}</p><div class="hero-meta"><span>${icon('calendar-days')}${parseDate(activity.start).getMonth() + 1} 月 ${parseDate(activity.start).getDate()} 日 ${weekNames[parseDate(activity.start).getDay()]}</span><span>${icon('clock-3')}${timeLabel(activity)}</span><span>${icon('map-pin')}${escapeHtml(activity.location)}</span><span>${icon('building-2')}${escapeHtml(activity.organizer)}</span></div></div><aside class="registration-card"><div class="capacity-title"><div><span>当前报名</span><br><strong>${count}</strong><span> / ${activity.capacity} 人</span></div><span>还剩 ${Math.max(0, activity.capacity - count)} 个名额</span></div><div class="progress"><span style="width:${percent}%"></span></div>${activity.status === 'DRAFT' ? `<button class="btn btn-primary" data-publish="${activity.id}">${icon('send')}发布活动</button>` : `<button class="btn ${joined ? '' : 'btn-primary'}" data-join="${activity.id}" ${!joined && (count >= activity.capacity || isEnded(activity)) ? 'disabled' : ''}>${joined ? `${icon('ticket-x')}取消报名` : `${icon('ticket-check')}立即报名`}</button>`}<small>${activity.status === 'DRAFT' ? '草稿不会出现在活动日历中' : isEnded(activity) ? '活动已经结束' : '活动开始前可取消报名'}</small></aside></section><div class="detail-grid"><section class="panel detail-panel"><h3>${icon('notebook-text')}活动介绍</h3><div class="detail-copy">${activity.description.split('\n').filter(Boolean).map((paragraph) => `<p>${escapeHtml(paragraph)}</p>`).join('')}</div></section><aside class="panel detail-panel"><div class="section-heading"><h3>${icon('users')}已报名同学</h3><span class="badge">${count} 人</span></div><p class="hint">仅展示可见的最小身份信息</p>${people.slice(0, 6).map((name) => `<div class="participant"><div class="avatar">${escapeHtml(name.slice(0, 1))}</div><div><strong>${escapeHtml(name)}</strong><span>2026级 · 校园用户</span></div>${name === '林夏' ? '<span class="badge badge-success" style="margin-left:auto">我的报名</span>' : ''}</div>`).join('') || '<p class="hint">暂未展示参与者</p>'}<button class="btn btn-soft" style="width:100%;margin-top:14px" data-show-participants>查看全部参与者</button></aside></div>`;
}
...
function renderGroupTargets(template) {
  ...
  row.insertAdjacentHTML('afterend', `<div class="input-group"><div class="section-heading"><label>目标群组</label><button type="button" class="btn btn-sm" data-manage-groups>${icon('settings-2')}管理我的群组</button></div><div class="group-picker">${allGroups().map((group) => `<label class="group-option"><input type="checkbox" name="targetGroups" value="${escapeHtml(group.id)}" ${selected.has(group.id) ? 'checked' : ''}><span><strong>${escapeHtml(group.name)}</strong><br><small>${escapeHtml(group.description)} · ${group.members.length} 人</small></span><span class="badge ${group.owned ? 'badge-success' : ''}">${group.owned ? '我的' : '管理员共享'}</span></label>`).join('')}</div><p class="hint">目标年级与群组成员取并集；发布时固定成员快照。</p></div>`);
}

function openGroupManager() {
  const modal = document.querySelector('#modal-root');
  modal.innerHTML = `<div class="modal-backdrop"><section class="modal" role="dialog" aria-modal="true" aria-labelledby="group-manager-title"><div class="modal-head"><div><span class="badge">共享学生群组</span><h2 id="group-manager-title">管理我的群组</h2></div><button class="btn btn-icon" data-close-modal aria-label="关闭">${icon('x')}</button></div><p class="hint">你创建的群组和管理员预设群组使用同一套数据，可直接作为活动目标。</p><div class="group-manager-list">${allGroups().map((group) => `<div class="group-manager-row"><div><strong>${escapeHtml(group.name)}</strong><p>${escapeHtml(group.description)} · ${group.members.join('、') || '暂无成员'} · ${group.owned ? '由我创建' : '管理员共享'}</p></div>${group.owned ? `<button class="btn btn-sm" data-delete-group="${escapeHtml(group.id)}">${icon('trash-2')}删除</button>` : '<span class="badge">只读</span>'}</div>`).join('')}</div><form id="group-form"><h3>新建群组</h3><div class="input-group"><label for="group-name">群组名称</label><input class="input" id="group-name" name="name" maxlength="80" required placeholder="例如：周末运动搭子"></div><div class="input-group"><label for="group-description">用途说明</label><input class="input" id="group-description" name="description" maxlength="200" placeholder="简要说明群组用途"></div><div class="input-group"><label>选择成员</label><div class="candidate-grid">${groupCandidates.map((name) => `<label><input type="checkbox" name="members" value="${escapeHtml(name)}">${escapeHtml(name)}<small>2026级</small></label>`).join('')}</div></div><div class="modal-actions"><button type="button" class="btn" data-close-modal>取消</button><button class="btn btn-primary" type="submit">${icon('plus')}创建群组</button></div></form></section></div>`;
  refreshIcons();
}
...
function createGroup(form) {
  ...
  customGroups.push({ id: `own-${Date.now()}`, name, description: String(data.get('description') || '').trim() || '我的活动目标群组', members: data.getAll('members'), owned: true });
  saveStoredState();
  openGroupManager();
